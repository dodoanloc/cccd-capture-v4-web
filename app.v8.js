...
async function saveRecord() {
  ...
  showSaveOverlay('Đang chuẩn bị lưu hồ sơ…', 'Đang đóng gói dữ liệu trước khi gửi.', 'info');
  await sleep(0);
  const formData = new FormData();
  ...
  const frontBlob = await dataUrlToBlob(frontSrc);
  const backBlob = await dataUrlToBlob(backSrc);
  ...
  try {
    ...
    const detail = err?.name === 'AbortError'
      ? 'Request save bị timeout sau 45 giây. Nếu hồ sơ vẫn xuất hiện ở web tra cứu thì backend đã lưu nhưng trình duyệt không nhận được phản hồi.'
      : String(err);
