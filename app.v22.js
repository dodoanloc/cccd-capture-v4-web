...
function ensureSaveOverlay() {
  let overlay = document.getElementById('runtimeSaveOverlay');
  if (overlay) return overlay;
  overlay = document.createElement('div');
  overlay.id = 'runtimeSaveOverlay';
  overlay.className = 'save-overlay hidden';
  overlay.innerHTML = `
    <div id="runtimeSaveOverlayCard" class="save-overlay-card info">
      <div id="runtimeSaveOverlayTitle" class="save-overlay-title">Đang xử lý...</div>
      <div id="runtimeSaveOverlayText" class="save-overlay-text">Vui lòng chờ trong giây lát.</div>
    </div>
  `;
...
function showSaveOverlay(title, text, type='info') {
  ...
  const card = document.getElementById('runtimeSaveOverlayCard');
  const titleEl = document.getElementById('runtimeSaveOverlayTitle');
  const textEl = document.getElementById('runtimeSaveOverlayText');
  titleEl.textContent = title || 'Đang xử lý...';
  textEl.textContent = text || 'Vui lòng chờ trong giây lát.';
  card.className = `save-overlay-card ${type}`;
  overlay.classList.remove('hidden');
  setSaveStatus(text || title || 'Đang lưu hồ sơ…', type);
}

function hideSaveOverlay() {
  const overlay = document.getElementById('runtimeSaveOverlay');
...
async function captureCurrentMode() {
  if (currentMode === 'front' || currentMode === 'back') {
    ...
    if (currentMode === 'front') {
      ...
      if (isLikelyMobile()) {
        frontUploadPromise = uploadSingleImageJson('front', blob)
          .then((path) => { frontUploadedPath = path; return path; })
          .catch((err) => { frontUploadedPath = ''; throw err; });
      }
    ...
    if (currentMode === 'back') {
      ...
      if (isLikelyMobile()) {
        backUploadPromise = uploadSingleImageJson('back', blob)
          .then((path) => { backUploadedPath = path; return path; })
          .catch((err) => { backUploadedPath = ''; throw err; });
      }
...
async function saveRecord() {
  ...
  try {
    ...
    if (isLikelyMobile()) {
      showSaveOverlay('Đang lưu hồ sơ…', 'Đang kiểm tra dữ liệu đã tải lên trước đó.', 'info');
      const frontPath = frontUploadedPath || await frontUploadPromise;
      const backPath = backUploadedPath || await backUploadPromise;
      if (!frontPath || !backPath) throw new Error('Ảnh CCCD chưa upload xong. Vui lòng chụp lại.');

      setSaveStatus('Đang ghi hồ sơ vào database...', 'info');
      showSaveOverlay('Đang lưu hồ sơ…', 'Đang ghi hồ sơ vào database.', 'info');
