...
function updateSaveButtonState() {
  ...
  const frontReady = !!frontUploadedPath;
  const backReady = !!backUploadedPath;
  if (frontReady && backReady) {
    els.saveBtn.textContent = 'Lưu hồ sơ';
    return;
  }
  els.saveBtn.textContent = 'Đang chuẩn bị lưu...';
}
...
function startVersionWatcher() {
  checkForAppUpdate();
  if (versionWatcherTimer) clearInterval(versionWatcherTimer);
  versionWatcherTimer = setInterval(checkForAppUpdate, VERSION_CHECK_INTERVAL_MS);
}
...
function startBackgroundImageUpload(kind, blob) {
  const runner = uploadSingleImageJson(kind, blob)
    .then((path) => {
      ...
      if (frontUploadedPath && backUploadedPath) {
        setSaveStatus('Ảnh đã upload xong. Bấm Lưu để ghi hồ sơ.', 'success');
      } else {
