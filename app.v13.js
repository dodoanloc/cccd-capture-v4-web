...
function showSaveOverlay(title, text, type='info') {
  setSaveStatus(`${title}${text ? `: ${text}` : ''}`, type);
}
...
function captureGuideFrameBlob(outputWidth = isLikelyMobile() ? 960 : 1600, quality = isLikelyMobile() ? 0.88 : 0.97) {
  const video = els.video;
...
async function saveRecord() {
  ...
  try {
    ...
    const detail = err?.name === 'AbortError'
      ? `Request save bị timeout sau ${isLikelyMobile() ? 60 : 45} giây. Nếu hồ sơ vẫn xuất hiện ở web tra cứu thì backend đã lưu nhưng trình duyệt không nhận được phản hồi.`
      : String(err);
    if (!isLikelyMobile()) {
