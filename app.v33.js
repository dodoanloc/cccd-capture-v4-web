...
async function startCamera() {
  try {
    stopCamera();
    const qrMode = currentMode === 'qr';
    stream = await navigator.mediaDevices.getUserMedia({
      video: {
        facingMode: useEnvironment ? { ideal: 'environment' } : 'user',
        width: { ideal: qrMode ? 1280 : 1280 },
        height: { ideal: qrMode ? 720 : 720 },
        frameRate: { ideal: qrMode ? 30 : 30, max: qrMode ? 30 : 30 }
      },
      audio: false
    });
...
async function scanQrFromVideoROI() {
  ...
  const quickPasses = [
    { roiRatio: 0.42, target: 280 },
    { roiRatio: 0.50, target: 360 },
    { roiRatio: 0.60, target: 420 },
  ];

  for (const pass of quickPasses) {
    const roiSize = Math.round(Math.min(vw, vh) * pass.roiRatio);
    const sx = Math.round((vw - roiSize) / 2);
    const sy = Math.round((vh - roiSize) / 2);
    canvas.width = pass.target;
    canvas.height = pass.target;
    ctx.drawImage(video, sx, sy, roiSize, roiSize, 0, 0, pass.target, pass.target);

    if (nativeBarcodeDetector) {
      try {
        const hits = await nativeBarcodeDetector.detect(canvas);
        const text = hits?.[0]?.rawValue || '';
        if (text) return { text, dataUrl: canvas.toDataURL('image/jpeg', 0.8), visualHint: true };
      } catch (_) {}
    }

    const raw = ctx.getImageData(0, 0, pass.target, pass.target);
    const quickDecoded = window.jsQR(raw.data, pass.target, pass.target, { inversionAttempts: 'attemptBoth' });
    if (quickDecoded?.data) {
      return { text: quickDecoded.data, dataUrl: canvas.toDataURL('image/jpeg', 0.8), visualHint: true };
    }
  }

  const roiRatios = [0.38, 0.48, 0.58, 0.68];
  const scales = [320, 420, 520];
  const passes = [
    { mode: 'raw' },
  ...
  for (const roiRatio of roiRatios) {
    ...
    for (const target of scales) {
      ...
      if (nativeBarcodeDetector && target >= 420) {
        try {
          const hits = await nativeBarcodeDetector.detect(canvas);
          const text = hits?.[0]?.rawValue || '';
          if (text) return { text, dataUrl: canvas.toDataURL('image/jpeg', 0.8), visualHint: true };
      ...
      if (decoded) {
        return { text: decoded, dataUrl: canvas.toDataURL('image/jpeg', 0.8), visualHint: true };
      }
    }
  }
  ...
  canvas.width = 260;
  canvas.height = 260;
  const fallbackRoi = Math.round(Math.min(vw, vh) * 0.5);
  ...
  let contrast = 0;
  for (let y = 2; y < 258; y += 10) {
    for (let x = 2; x < 258; x += 10) {
      const idx = (y * 260 + x) * 4;
      contrast += Math.abs(d[idx] - d[idx + 4]) + Math.abs(d[idx] - d[idx + 260 * 4]);
    }
  }
  return { text: '', dataUrl: '', visualHint: contrast > 8500 };
}
...
function clearUploadPromise(kind) {
  if (kind === 'front') frontUploadPromise = null;
  if (kind === 'back') backUploadPromise = null;
}
...
async function ensureUploadedImage(kind, blob) {
  ...
  if (currentPromise) {
    try {
      const existing = await withTimeout(currentPromise, 8000, `upload ${kind}`);
  ...
  return withTimeout(startBackgroundImageUpload(kind, blob), 15000, `retry upload ${kind}`);
}
...
async function saveRecord() {
  ...
  try {
    if (isLikelyMobile()) {
      setSaveStatus('Đang tải và kiểm tra đủ 2 ảnh CCCD...', 'info');
