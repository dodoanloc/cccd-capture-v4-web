...
async function saveRecord() {
  ...
  try {
    ...
    if (isLikelyMobile()) {
      setSaveStatus('Đang tải ảnh mặt trước...', 'info');
      showSaveOverlay('Đang lưu hồ sơ…', 'Đang tải ảnh mặt trước lên server.', 'info');
      const frontPath = await uploadSingleImageJson('front', frontBlob);

      setSaveStatus('Đang tải ảnh mặt sau...', 'info');
      showSaveOverlay('Đang lưu hồ sơ…', 'Đang tải ảnh mặt sau lên server.', 'info');
      const backPath = await uploadSingleImageJson('back', backBlob);

      setSaveStatus('Đang ghi hồ sơ vào database...', 'info');
      showSaveOverlay('Đang lưu hồ sơ…', 'Ảnh đã tải xong, đang ghi hồ sơ vào database.', 'info');
