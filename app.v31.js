  ...
  try {
    ...
    stream = await navigator.mediaDevices.getUserMedia({
      video: {
        ...
        width: { ideal: qrMode ? 960 : 1280 },
        height: { ideal: qrMode ? 540 : 720 },
        frameRate: { ideal: qrMode ? 24 : 30, max: qrMode ? 24 : 30 }
      },
      audio: false
    });
...
async function scanQrFromVideoROI() {
  ...
  const quickPasses = [
    { roiRatio: 0.48, target: 260 },
    { roiRatio: 0.58, target: 320 },
  ];
  ...
  for (const pass of quickPasses) {
    ...
    if (nativeBarcodeDetector) {
      try {
        ...
        if (text) return { text, dataUrl: canvas.toDataURL('image/jpeg', 0.76), visualHint: true };
      } catch (_) {}
    }

    const quickDecoded = window.jsQR(ctx.getImageData(0, 0, pass.target, pass.target).data, pass.target, pass.target);
    if (quickDecoded?.data) {
      return { text: quickDecoded.data, dataUrl: canvas.toDataURL('image/jpeg', 0.76), visualHint: true };
    }
  }

  const roiRatios = [0.44, 0.56];
  const scales = [280, 360];
  const passes = [
    { mode: 'raw' },
  ...
  for (const roiRatio of roiRatios) {
    ...
    for (const target of scales) {
      ...
      if (decoded) {
        return { text: decoded, dataUrl: canvas.toDataURL('image/jpeg', 0.76), visualHint: true };
      }
    }
  }
  ...
  canvas.width = 180;
  canvas.height = 180;
  const fallbackRoi = Math.round(Math.min(vw, vh) * 0.48);
  ctx.drawImage(video, Math.round((vw - fallbackRoi) / 2), Math.round((vh - fallbackRoi) / 2), fallbackRoi, fallbackRoi, 0, 0, 180, 180);
  const imageData = ctx.getImageData(0, 0, 180, 180);
  const d = imageData.data;
  let contrast = 0;
  for (let y = 2; y < 178; y += 12) {
    for (let x = 2; x < 178; x += 12) {
      const idx = (y * 180 + x) * 4;
      contrast += Math.abs(d[idx] - d[idx + 4]) + Math.abs(d[idx] - d[idx + 180 * 4]);
    }
  }
  return { text: '', dataUrl: '', visualHint: contrast > 5000 };
}
